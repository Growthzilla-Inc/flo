...
(function() {
  ...
  module.exports = {
    ...
    'test add_term': function() {
      ...
      return flo.add_term(term_type, term_id, term, term_score, term_data, function() {
        return async.parallel([
          (function(callback) {
            return flo.redis.hget(flo.key(term_type,
          "data"),
          term_id,
          function(err,
          reply) {
              ...
              assert.equal(term,
          result.term);
              assert.equal(term_score,
          result.score);
              assert.eql(term_data,
          result.data);
              return callback();
            });
          }),
          (function(callback) {
            return async.map(flo.prefixes_for_phrase(term),
          ((w,
          cb) => {
              return flo.redis.zrange(flo.key(term_type,
          "index",
          w),
          0,
          -1,
    ...
    'test search_term': function() {
      ...
      return async.series([
        (function(callback) {
          return async.forEach(venues,
        (function(venue,
        cb) {
            return flo.add_term("venues",
        venue.id,
        venue.term,
        venue.score,
        venue.data,
    ...
    'test remove_term': function() {
      ...
      return async.series([
        ...
        (function(next) {
          return flo.remove_term(term_type,
        term_id,
        next);
        }),
        (function(next) {
        ...
        (function(next) {
          return flo.search_term([term_type],
        term,
        function(err,
        result) {
            var eql;
